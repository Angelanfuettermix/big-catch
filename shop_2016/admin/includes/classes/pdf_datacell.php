...
//echo "mk1 h=".$this->height."    bord=".$this->_border."<br>\n";    
  ...
  function _content() {
    ...
    // -- h.koch for alkim-media -- 03.2016 --     Patch für Lieferadresse
    $ret['receiver'].= "\n\n\n".PDFBILL_SHIPPING_ADDRESS; 
...
//echo "<pre>"; print_r($this->products_data); echo "</pre>";
  ...
  // die in html gespeicherten Attributwerte vom html befreien 
  function products_attributes_strip_tags($products_attributes) {
    $products_attributes = str_replace('</div>', "\n", $products_attributes);
    $products_attributes = str_replace('<br>', "\n", $products_attributes);
    $products_attributes = str_replace('<br />', "\n", $products_attributes);
    $products_attributes = str_replace('<br/>', "\n", $products_attributes);
    $products_attributes = strip_tags($products_attributes);
    $products_attributes = html_entity_decode($products_attributes);
...
//echo "this->products_data['order_data'][$pos_inx]['PRODUCTS_MODEL']=".$this->products_data['order_data'][$pos_inx]['PRODUCTS_MODEL']."<br>\n";
    ...
    //$txt = str_replace( '*p_name*',         $this->products_data['order_data'][$pos_inx]['PRODUCTS_NAME'].$this->products_data['order_data'][$pos_inx]['PRODUCTS_ATTRIBUTES'], $txt );
    $products_attributes = $this->products_data['order_data'][$pos_inx]['PRODUCTS_ATTRIBUTES'];
    $products_attributes = $this->products_attributes_strip_tags($products_attributes);
    $txt = str_replace( '*p_name*',         $this->products_data['order_data'][$pos_inx]['PRODUCTS_NAME'].$products_attributes, $txt );
...
//echo "i= $i txt=".$text."<br>\n";                                  
      ...
      //$pdf->Cell( $width, $line_h, $text, $this->_border, 2, $this->position );
$pdf->Cell( $width, $line_h, $text, 'B', 2, $this->position );
...
//echo "<pre>"; print_r($this->products_data['order']->customer['vat_id']); echo "</pre>";
