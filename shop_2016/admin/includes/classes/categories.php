...
define('ADD_PRODUCTS_DESCRIPTION_FIELDS','products_order_description,products_meta_robots,products_meta_date,products_meta_google_site_verification'); //products_order_description
...
class categories {
    ...
    function unArrayCleanImgName($Input) { //GUNNART
        ...
        while ( $Count < count($Input) ) {
            $Name .= $this->cleanImgName($Input[$Count]);
            $Count++;
        }
        return $Name;
    }
  ...
  function insert_category($categories_data, $dest_category_id, $action = 'insert') {
    ...
    $sql_data_array = am_fields::add_cat_fields($sql_data_array, $categories_data, $lang['id']);
      ...
      //GUNNART // $categories_image_name = $categories_id.'.'.$cnsuffix;
        $categories_image_name = $categories_id.'_'.$this->unArrayCleanImgName($cname_arr).'.'.strtolower($cnsuffix); //GUNNART
        //GUNNART // @ unlink(DIR_FS_CATALOG_IMAGES.'categories/'.$categories_image_name);
        @ unlink(DIR_FS_CATALOG_IMAGES.'categories/'.$categories_data['categories_previous_image']); //GUNNART
  ...
  */
  ...
  function insert_product($products_data, $dest_category_id, $action = 'insert') {
    ...
    $sql_data_array = am_fields::add_product_fields($sql_data_array, $products_data);
    if($sql_data_array["products_master_mixxxer"]==1){
        $q = "UPDATE products SET products_master_mixxxer = 0";
        xtc_db_query($q);
    }
    ...
    //prepare products_image filename
      ...
      //GUNNART // $products_image_name = $products_id.'_0.'.$nsuffix;
      $products_image_name = $products_id.'_0_'.$this->unArrayCleanImgName($pname_arr).'.'.strtolower($nsuffix); //GUNNART
    ...
    for ($img = 0; $img < MO_PICS; $img ++) {
      if ($pIMG = xtc_try_upload('mo_pics_'.$img, DIR_FS_CATALOG_ORIGINAL_IMAGES, '777', $accepted_mo_pics_image_files_extensions, $accepted_mo_pics_image_files_mime_types)) {
        ...
        $nsuffix = array_pop($pname_arr);
        //GUNNART //$products_image_name = $products_id.'_'. ($img +1).'.'.$nsuffix;
        $products_image_name = $products_id.'_'.($img +1).'_'.$this->unArrayCleanImgName($pname_arr).'.'.strtolower($nsuffix); //GUNNART
        $dup_check_query = xtc_db_query("SELECT COUNT(*) AS total
                                                      FROM ".TABLE_PRODUCTS_IMAGES."
    ...
    foreach ($languages AS $lang) {
      ...
      $sql_data_array = array('products_name' => xtc_db_prepare_input($products_data['products_name'][$language_id]),
        ...
        $sql_data_array = am_fields::add_product_fields($sql_data_array, $products_data, $language_id);
