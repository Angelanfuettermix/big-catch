...
if ($_GET['action'] != 'delete_product_option') {
  ...
  if (isset($_GET['searchoption'])) {
    $options = "-- products_attributes.php
      SELECT * FROM " . TABLE_PRODUCTS_OPTIONS . " WHERE language_id = '" . $_SESSION['languages_id'] . "' AND products_options_id != ".(int)CONFIG_OPT_GROUP." AND products_options_name LIKE '%" . $_GET['searchoption'] . "%' ORDER BY " . $option_order_by;
  } else {
    $options = "-- products_attributes.php
      SELECT * FROM " . TABLE_PRODUCTS_OPTIONS . " WHERE language_id = '" . (int)$_SESSION['languages_id'] . "' AND products_options_id != ".(int)CONFIG_OPT_GROUP." ORDER BY " . $option_order_by;
  }
  if (!$option_page) {
    $option_page = 1;
  }
