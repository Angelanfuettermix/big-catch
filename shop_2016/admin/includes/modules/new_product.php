...
/* --------------------------------------------------------------
 ...
 defined( '_VALID_XTC' ) or die( 'Direct Access to this location is not allowed.' );
  ...
  if (isset($_GET['pID']) && (!$_POST)) {
    ...
    $products_meta_robots = $_POST['products_meta_robots'];
    $products_meta_date = $_POST['products_meta_date'];
    $products_meta_google_site_verification = $_POST['products_meta_google_site_verification'];
...
include('mixxxer_manager/am_html_header.php');
require('mixxxer_manager/am_const.php');
?>
...
<table bgcolor="f3f3f3" style="width: 100%; border: 1px solid; border-color: #aaaaaa; padding:5px;">
  <tr>
    ...
    <?php am_fields::draw_product_fields($pInfo); ?>
    <?php
    if($pInfo->products_is_mixxxer == 1){
    $_SESSION["pid"] = $pInfo->products_id;
    if ($pInfo->products_id !="" ){
    echo '<a href="#" class="am_show_manager button">'.AM_SHOW_MANAGER.'</a>';
    echo '<div style="padding:20px;" class="am_man_wr"><br /></div>';
    }
    }

    if($_GET["extra"]=="mixxxer" && ($_GET["pID"]=='' || $_GET["pID"]=='0')){
    ?>
    <script>
    $(document).ready(function(){
    $('#products_is_mixxxer_1').click();
    $('#products_master_mixxxer_1').click();
    ...
    $('#products_master_mixxxer_1').click(function(){
    $('#products_is_mixxxer_1').click();
    });
    $('#products_is_mixxxer_0').click(function(){
    $('#products_master_mixxxer_0').click();
    });
    });
    </script>
      ...
      <div style="width: 860px; padding:5px;clear:both;">
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          ...
          <div class="main" valign="top" style="padding: 3px; line-height:20px;">
              <?php echo $lng_image. '&nbsp;Robots: (max. 255 '. TEXT_CHARACTERS .')'; ?> <br/>
              <?php echo xtc_draw_input_field('products_meta_robots[' . $languages[$i]['id'] . ']',(isset($products_meta_robots[$languages[$i]['id']]) ? stripslashes($products_meta_robots[$languages[$i]['id']]) : "index,follow"), 'style="width:100%" maxlength="255"'); ?><br/>
              ...
              <?php echo $lng_image. '&nbsp;Date: (Format: 2016-01-25)'; ?> <br/>
              <?php echo xtc_draw_input_field('products_meta_date[' . $languages[$i]['id'] . ']',(isset($products_meta_date[$languages[$i]['id']]) ? stripslashes($products_meta_date[$languages[$i]['id']]) : $products_desc_fields['products_meta_date']), 'style="width:100%" maxlength="255"'); ?><br/>
              <?php echo $lng_image. '&nbsp;Google-Site-Verification: (max. 255 '. TEXT_CHARACTERS .')'; ?> <br/>
              <?php echo xtc_draw_input_field('products_meta_google_site_verification[' . $languages[$i]['id'] . ']',(isset($products_meta_google_site_verification[$languages[$i]['id']]) ? stripslashes($products_meta_google_site_verification[$languages[$i]['id']]) : $products_desc_fields['products_meta_google_site_verification']), 'style="width:100%" maxlength="255"'); ?><br/>
          </div>
          <?php
          echo ('</div>');
        } ?>
