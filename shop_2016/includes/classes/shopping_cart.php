  ...
  function calculate() {
    ...
    while (list ($products_id) = each($this->contents)) {
      ...
      if ($product = xtc_db_fetch_array($product_query)) {
        ...
        if($this->contents[$products_id]["attributes"][CONFIG_OPT_GROUP] != ""){

            $mix_id = $_SESSION["mixxxer"]->getMixxxIdFromPovId($this->contents[$products_id]["attributes"][CONFIG_OPT_GROUP]);
            $this_mix = $_SESSION["mixxxer"]->loadMixxx($mix_id);
            $products['products_name'] = $this_mix->name;
            //$product['products_weight'] = $this->weight;
            if(is_object($this_mix)){
                $products_price = $this_mix->calc_price($qty, $mix_id);
                $product['products_weight'] += $this_mix->calc_weight();
            }
    ...
    while (list ($products_id,) = each($this->contents)) {
      if($this->contents[$products_id]['qty'] != 0 || $this->contents[$products_id]['qty'] !=''){
        ...
        if ($products = xtc_db_fetch_array($products_query)) {          
           if ($products['products_status'] == 0 && $products["products_is_mixxxer"]==0) {
            ...
            $products_price = $xtPrice->xtcGetPrice($products['products_id'],
                                ...
                                if($this->contents[$products_id]["attributes"][CONFIG_OPT_GROUP] != ""){
                                    ...
                                    $products['products_name'] = $this_mix->name.' (ID: '.$_SESSION["mixxxer"]->getMixxxIdFromPovId($this->contents[$products_id]["attributes"][CONFIG_OPT_GROUP]).')';
                                    if(is_object($this_mix))
                                    $products_price = $this_mix->calc_price($this->contents[$products_id]['qty'], $mix_id);
