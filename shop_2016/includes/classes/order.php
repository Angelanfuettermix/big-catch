  ...
  class order {
        ...
        if (xtc_db_num_rows($attributes_query)) {
          while ($attributes = xtc_db_fetch_array($attributes_query)) {
            $this->products[$index]['attributes'][$subindex] = array('option' => $attributes['products_options'],
                                                                     ...
                                                                     'label'=>!empty($attributes['orders_products_attributes_label_html'])
                                                                     );
    ...
    function cart() {
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        ...
        if($products[$i]["attributes"][CONFIG_OPT_GROUP] != ""){
            $mix_id = $_SESSION["mixxxer"]->getMixxxIdFromPovId($products[$i]["attributes"][CONFIG_OPT_GROUP]);
            $this_mix = $_SESSION["mixxxer"]->loadMixxx($mix_id);
            $products[$i]['products_name'] = $this_mix->name;
            $products_price = $this_mix->calc_price($products[$i]['quantity'], $mix_id);
            $products[$i]['weight'] += $this_mix->calc_weight();
