...
class product {
  ...
  * Constructor
   ...
   * @return product
   */
   function product($pID = 0, $ignoreStatus = false) {
    ...
    $product_query = "SELECT *
                        ...
                        JOIN ".TABLE_PRODUCTS_DESCRIPTION." AS pd ON ".($ignoreStatus?'':"p.products_status = '1' and ")."
  ...
  function buildDataArray(&$array,$image='thumbnail') {
    ...
    */
    ...
    /*
    FOR REASONS OF BETTER PERFORMANCE IT IS RECOMMENDED
    TO REQUEST products_mix_id IN THE ORIGINAL query INSTEAD OF
    THIS PLACE
    */
    $q = "SELECT * FROM products WHERE products_id = ".$array['products_id'];
    $rs = xtc_db_query($q);
    $r = mysql_fetch_object($rs);
    $array['products_mix_id'] = $r->products_mix_id;



    if($array['products_mix_id']>0){
    if(is_object($_SESSION["mixxxer"])){
    $my_mix = $_SESSION["mixxxer"]->loadMixxx($array['products_mix_id']);

    if(is_object($my_mix)){
    if($r->products_price_from_mixxx==1){
    $products_price['formated'] = $xtPrice->xtcFormat($my_mix->calc_price(), true);
    }

    if($r->products_overwrite_name==1){
    $array['products_name'] = $my_mix->name;
    }

    $array["bp"] = $my_mix->give_base_price($xtPrice);
    }
    }
