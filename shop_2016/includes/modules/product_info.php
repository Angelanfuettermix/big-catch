...
if (!is_object($product) || !$product->isProduct()) {
  ...
  if ($_SESSION['customers_status']['customers_status_show_price'] != '0'
    ...
    $info_smarty->assign('ADD_QTY', xtc_draw_input_field('products_qty', '1', ($hide_qty ? '' : 'size="1"'), ($hide_qty ? 'hidden' : 'text')).' '.$add_pid_to_qty);
  ...
  include (DIR_WS_MODULES.FILENAME_CROSS_SELLING);
    if($product->data['products_mix_id']>0){
        $my_mix = $_SESSION["mixxxer"]->loadMixxx($product->data['products_mix_id']);
    if(MIXXXER_SHOW == 1){
        $info_smarty->assign('EDIT_MIX_LINK', xtc_href_link('mixxxer.php', 'mix_id='.$product->data['products_mix_id']));
        $info_smarty->assign('IS_MIX', 1);
        $info_smarty->assign('CONFIG_LIST', $my_mix->give_item_list_fancy());
    }

        $bp = $my_mix->give_base_price($xtPrice);

        $info_smarty->assign('BASE_PRICE', $bp);

        $info_smarty->assign('MIX_ID_HIDDEN', '<input type="hidden" value="'.$product->data['products_mix_id'].'" name="mix_id" />');

    if($product->data['products_price_from_mixxx']==1){
    $info_smarty->assign('PRODUCTS_PRICE', $xtPrice->xtcFormat($my_mix->calc_price(), true));
    }
    if($product->data['products_overwrite_name']==1){
        $info_smarty->assign('PRODUCTS_NAME', $my_mix->name);
    }
    $info_smarty->assign('REMIXXX_ALLOWED', $product->data['products_remixxx']);
