...
for ($i = 0, $n = sizeof($products); $i < $n; $i ++) {
  ...
  if (isset ($products[$i]['attributes']) && is_array($products[$i]['attributes'])) {
    ...
    if($option == CONFIG_OPT_GROUP){

        $this_mix = $_SESSION["mixxxer"]->loadMixxx($_SESSION["mixxxer"]->getMixxxIdFromPovId($value));
        $attributes['products_options_name'] = CONFIGURATION;
    if(is_object($this_mix))
        $attributes['products_options_values_name'] = $this_mix->give_item_list_plain();
    }
