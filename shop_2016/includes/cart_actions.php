  ...
  switch ($action) {
    ...
    case 'add_product':
      if (isset ($_POST['products_id']) && is_numeric($_POST['products_id'])) {
                ...
                if($_POST["mix_id"]){
                    $_SESSION["mixxxer"]->addMixxxToCart($_POST["mix_id"], xtc_remove_non_numeric($_POST['products_qty']));
    ...
    case 'buy_now':
      if (isset($_GET['BUYproducts_id'])) {
        ...
        $q = "SELECT * FROM products WHERE products_id = ".(int)$_GET['BUYproducts_id'];
        $rs = xtc_db_query($q);
        $r = mysql_fetch_object($rs);

        if($r->products_mix_id){
        $_SESSION["mixxxer"]->addMixxxToCart($r->products_mix_id);
