...
if(!file_exists($cache_path) || filemtime($cache_path) < (time()+14400)){
    ...
    $minifier->add($base_path.'fancybox/jquery.fancybox-1.3.4.js');
...
<?php if (!strstr($PHP_SELF, 'mixxxer.php' )) { ?>
    <script src="<?php echo 'templates/'.CURRENT_TEMPLATE; ?>/javascript/jquery.form.js" type="text/javascript"></script>
    <script src="<?php echo 'templates/'.CURRENT_TEMPLATE; ?>/javascript/jquery-ui-1.7.1.custom.min.js" type="text/javascript"></script>
    <script src="<?php echo 'templates/'.CURRENT_TEMPLATE; ?>/javascript/mod_functions.js" type="text/javascript"></script>
    <?php
    $ajaxrequest_type = (getenv('HTTPS') == '1' || getenv('HTTPS') == 'on') ? 'SSL' : 'NONSSL';
    if($ajaxrequest_type == 'SSL'){
        echo '<script type="text/javascript"><!--';
        echo 'mod_ajax_request = "SSL";'."\n";
        echo 'mod_ajax_normal_server = "'.HTTP_SERVER.'";'."\n";
        echo 'mod_ajax_ssl_server = "'.HTTPS_SERVER.'";'."\n";
        echo '//--></script>';
    }
}
?>

<!-- write $_PHP_SELF to $PHP_SELF (DK) -->
<?php if (strstr($PHP_SELF, 'mixxxer.php' )) { ?>
...
<script type="text/javascript" src="<?php echo 'templates/'.CURRENT_TEMPLATE; ?>/javascript/elevatezoom/jquery.elevateZoom.min.js"></script>
...
$(document).on('mouseover', '.mixxxer_item_thumb', function(){
    $(this).elevateZoom({
        scrollZoom: true,
        borderSize: 0,
        lensBorder: 0,
