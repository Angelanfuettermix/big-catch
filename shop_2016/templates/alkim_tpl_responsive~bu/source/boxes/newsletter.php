...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_newsletter.html', $cache_id) || !$cache) {
  ...
  $box_smarty->assign('FIELD_EMAIL',xtc_draw_input_field('email', '', 'maxlength="50" placeholder="' . $box_smarty->_config[0]['vars']['placeholder_newsletter'] . '"'));
  $box_smarty->assign('BUTTON',xtc_image_submit('button_login_newsletter.gif', IMAGE_BUTTON_LOGIN));
  $box_smarty->assign('FORM_END','</form>');
}

$smarty->assign('box_NEWSLETTER', $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_newsletter.html', $cache_id));
$smarty->assign('box_NEWSLETTER_CENTER', $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_newsletter_center.html', $cache_id));
?>