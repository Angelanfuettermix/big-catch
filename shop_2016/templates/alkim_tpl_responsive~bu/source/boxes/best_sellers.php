  ...
  if (count($box_content) > 0) {
    ...
    if (!$cache) {
      if ($box_content!='') {
          ...
          $box_best_sellers_center = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_best_sellers_center.html');
      }
    } else {
      $box_best_sellers = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_best_sellers.html', $cache_id);
      $box_best_sellers_center = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_best_sellers_center.html', $cache_id);
    }
    $smarty->assign('box_BESTSELLERS', $box_best_sellers);
    $smarty->assign('box_BESTSELLERS_CENTER', $box_best_sellers_center);
  }
}
//EOF - DokuMan - 2010-07-12 - fix Smarty cache error on unlink
?>
