  ...
  for ($i = 0, $n = sizeof($order->products); $i < $n; $i ++) {
    ...
    if (isset ($order->products[$i]['attributes'])) {
      ...
      for ($j = 0, $n2 = sizeof($order->products[$i]['attributes']); $j < $n2; $j ++) {
        ...
        $label_html = '';
        if($order->products[$i]['attributes'][$j]['option_id'] == CONFIG_OPT_GROUP){
            $mix_id = $_SESSION["mixxxer"]->getMixxxIdFromPovId($order->products[$i]['attributes'][$j]['value_id']);
            $this_mix = $_SESSION["mixxxer"]->loadMixxx($mix_id);
            $this_mix->decrementProducts($order->products[$i]['qty']);
            $attributes_values['products_options_name'] = CONFIGURATION;
            $attributes_values['products_options_values_name'] = '<br />'.$this_mix->give_item_list_plain();
            $label_html = $this_mix->getLabelHtml($mix_id);
        }
        ...
        $sql_data_array = array ('orders_id' => $insert_id,
                                 ...
                                 'orders_products_attributes_label_html'=>$label_html
                                );
