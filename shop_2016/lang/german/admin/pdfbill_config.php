...
define( PDFBILL_SHIPPING_ADDRESS, 'Lieferadresse:');
...
function texts_html( $texts ) {
  if( is_array($texts) ) {
  	foreach ($texts as $key => $val) {
  		if( @strpos($texts[$key],'helptext')===false || is_array($texts[$key]) ) {    // labels with 'helptext' no proceed
  			$texts[$key]=texts_html($texts[$key]);
  		}
  	}
    return $texts;
  } else {
