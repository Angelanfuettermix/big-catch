...
class mixxx {
    ...
    function add_item($mi_id, $qty = 1, $absolute = false) {
        ...
        $this->calc_by_group();
        $q  = "SELECT * FROM mixxxer_items mi, mixxxer_items_to_mixxxer_groups mi2mg, mixxxer_groups mg, mixxxer_items_active mia WHERE 
              mi.mi_id = " . (int) $mi_id . " 
    ...
    function give_item_list_plain() {
        ...
        foreach ($by_group AS $group => $items) {
            ...
            foreach ($items AS $k => $v) {
                if ($v["id"]) {
                    if ($prev_group != $v["mg_supergroup"] && $v["mg_supergroup"] != '') {
                        $ret .= '<div class="fl_line"><b>' . $v["mg_supergroup"] . '</b></div>' . "\n";
                    }
                    ...
                    $ret .= $v['name'] . (($v['qty'] > 1) ? ' (' . $v['qty'] . 'g)' : '');
                    if ($v["model"] != '') {
                        $ret .= '<i> - ' . $v["model"] . '</i>';
                    }
                    $ret .= '<br/>' . "\n";
                    $ret .= ($text ? '<i>' . $text . '</i><br />' . "\n" : '');
                    $ret .= ($files[$v["id"]] ? '<i><a href="' . $files[$v["id"]]["path"] . '" target="_blank">' . $files[$v["id"]]["name"] . '</a></i><br />' . "\n" : '');
