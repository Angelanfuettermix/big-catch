...
foreach($p["attributes"] AS $j=>$att){
if($att["option_id"] == CONFIG_OPT_GROUP){
$this_mix = $_SESSION["mixxxer"]->loadMixxx($_SESSION["mixxxer"]->getMixxxIdFromPovId($order->products[$i]['attributes'][$j]['value_id']));
$order->products[$i]['attributes'][$j]['option'] = CONFIGURATION;
$order->products[$i]['attributes'][$j]['value'] = '<br />'.$this_mix->give_item_list_plain();
}
}
}
}
